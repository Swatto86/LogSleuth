...
/// Regression: VBR 12.x logs use 4-space column-aligned format (]    <thread>    Level).
/// Before the fix, \]\s< only accepted a single space so these files fell through to
/// plain-text despite their filenames matching VBR patterns.
///
/// This test verifies both:
///   (a) content-only detection (for files with no recognisable name)
///   (b) production filename detection with Svc.VeeamBackup.log (the file shown
///       in the screenshot as "plain-text (30%)")
#[test]
fn e2e_vbr_v12_multispace_format_detects_and_parses() {
    let profiles = load_profiles();
    ...
    // Real VBR 12.x log lines: 4 spaces between ] and <, 4 spaces between > and level.
    let new_fmt_lines: Vec<String> = vec![
        "[04.01.2026 07:00:50.526]    <13>    Info (4)            [AP] output: --batchex:0"
            .to_string(),
        "[04.01.2026 07:00:50.526]    <31>    Info (4)            [AP] output: --size: 2747657"
            .to_string(),
        "[04.01.2026 07:00:51.001]    <13>    Warning (3)         Transfer rate below threshold"
            .to_string(),
        "[04.01.2026 07:00:52.100]    <14>    Error (1)           Failed to connect to repository"
            .to_string(),
        "[04.01.2026 07:00:53.200]    <15>    Info (4)            Retry succeeded".to_string(),
    ];

    // (a) Content-only detection: name does not match any VBR file_pattern.
    let content_only = profile::auto_detect("arbitrary.log", &new_fmt_lines, &profiles);
    assert!(
        content_only.is_some(),
        "VBR 12.x content must be detectable even without a matching filename"
    );
    assert_eq!(
        content_only.unwrap().profile_id,
        "veeam-vbr",
        "content-only detection must select veeam-vbr"
    );

    // (b) Production filename: Svc.VeeamBackup.log (matches Svc.Veeam*.log pattern).
    let named = profile::auto_detect("Svc.VeeamBackup.log", &new_fmt_lines, &profiles);
    assert!(
        named.is_some(),
        "Svc.VeeamBackup.log with VBR 12.x content must auto-detect"
    );
    let named_det = named.unwrap();
    assert_eq!(
        named_det.profile_id, "veeam-vbr",
        "Svc.VeeamBackup.log must detect as veeam-vbr, got '{}'",
        named_det.profile_id
    );

    // (c) Parsing must produce at least one entry per log line (no 0-entry fallback).
    let content = new_fmt_lines.join("\n");
    let dummy_path = std::path::Path::new("Svc.VeeamBackup.log");
    let parse_result = parse_content(
        &content,
        dummy_path,
        vbr_profile,
        &ParseConfig::default(),
    ...
    assert!(
        !parse_result.entries.is_empty(),
        "VBR 12.x lines must produce parsed entries, not fall through to plain-text"
    );
    assert_eq!(
        parse_result.entries.len(),
        new_fmt_lines.len(),
        "each VBR 12.x line should parse as one entry"
    );

    // Verify severity extraction works for the multi-space format.
    use logsleuth::core::model::Severity;
    let has_error = parse_result
        .entries
        .iter()
        .any(|e| e.severity == Severity::Error);
    let has_warning = parse_result
        .entries
        .iter()
        .any(|e| e.severity == Severity::Warning);
    assert!(
        has_error,
        "Error severity must be extracted from VBR 12.x lines"
    );
    assert!(
        has_warning,
        "Warning severity must be extracted from VBR 12.x lines"
    );
}
...
fn e2e_parse_veeam_vbr_sample() {
    ...
    // The fixture contains old-format (single-space) and new-format (VBR 12.x,
    // 4-space) entries. With multiline continuation the stack-trace lines fold
    // into their preceding entry and the startup header block becomes a
    // continuation of the last old-format entry, so we expect fewer entries
    // than raw lines. Assert conservatively: at least 10 primary entries.
    assert!(
