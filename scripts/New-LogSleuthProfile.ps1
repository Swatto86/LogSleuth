...
.DESCRIPTION
    ...
    The emitted schema is [profile] / [detection] / [parsing] / [severity_override],
    matching profiles/*.toml. The severity capture group must be named 'level'.
...
$firstTokens = $sampledLines |
    ForEach-Object { (($_ -split '\s+') | Select-Object -First 3) -join ' ' } |
...
if (-not $contentMatch) {
    # Fallback: use the detected timestamp pattern as the content anchor.
    # This is already a regex, so it must NOT be regex-escaped again.
    $contentMatch = ($timestampCandidates[0].Pattern.TrimStart('^'))
...
$lines.Add("description = `"Generated by New-LogSleuthProfile.ps1`"")
$lines.Add("# log_locations is an ARRAY of strings, e.g.:")
...
$lines.Add("")
$lines.Add("[parsing]")
$lines.Add("# line_pattern: named capture groups 'timestamp', 'level', 'thread', 'component', 'message'.")
$lines.Add("# NOTE: the severity group MUST be named 'level' -- LogSleuth ignores a group named 'severity'.")
if ($bestCapture) {
    $linePattern = $bestCapture + "(?P<message>.*)"
} else {
    $linePattern = "(?P<message>.*)"
}
if ($tsConfident) {
...
$lines.Add("multiline_mode = `"continuation`"")

if ($foundSeverity.Count -gt 0) {
    $lines.Add("")
    $lines.Add("# severity_override: REGEX patterns matched against the message text when")
...
$lines.Add("")
$lines.Add("# severity_mapping maps LITERAL level strings captured by (?P<level>...) to severities:")
$lines.Add("# [severity_mapping]")
$lines.Add("# critical = [`"FATAL`"]")
$lines.Add("# error    = [`"ERROR`", `"ERR`"]")
$lines.Add("# warning  = [`"WARN`"]")
$lines.Add("# info     = [`"INFO`"]")
$lines.Add("# debug    = [`"DEBUG`", `"TRACE`"]")
...
Write-Host "  NOTE: profiles produced by versions of this script before the [detection]/" -ForegroundColor Yellow
Write-Host "        [parsing] schema fix will not load. Regenerate them with this script." -ForegroundColor Yellow
Write-Host ""
