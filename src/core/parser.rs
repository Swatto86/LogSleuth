...
pub fn parse_content(
    ...
    // Hoist allocations that are identical for every entry out of the loop.
    // On a 100 k-line file this eliminates 100 k PathBuf clones and 100 k
    // String clones for profile_id.
    let source_file = file_path.to_path_buf();
        ...
        if let Some(caps) = profile.line_pattern.captures(line) {
            ...
            match profile.multiline_mode {
                ...
                crate::core::model::MultilineMode::Raw => {
                    // Create an unparsed entry.  Reuse the message String as
                    // raw_text by cloning it once rather than calling
                    // line.to_string() twice (two separate heap allocations for
                    // identical content).
                    let message = line.to_string();
                    let raw_text = message.clone();
    ...
    // -------------------------------------------------------------------------
    // Post-loop pass: enforce entry size caps + timestamp sniff fallback.
    //
    // These two concerns are combined into a single O(n) traversal to avoid
    // iterating `entries` twice (previously two separate loops).
    //
    // Truncation guard: the in-loop check only fires on continuation appends.
    // A single matching line longer than max_entry_size (e.g. a 100 KB JSON
    // blob) would escape it entirely.  The post-loop pass guarantees every
    // entry is bounded regardless of origin.
    //
...
fn sniff_timestamp_hinted(
    ...
    let sniffers = SNIFFERS.get_or_init(|| {
        ...
        let candidates: Vec<Option<Sniffer>> = vec![
            ...
            // ------------------------------------------------------------------
            // Tier 13 -- Unix epoch milliseconds (13 digits, at line start).
            ...
            try_re(r"^\d{13}(?:\.\d+)?").map(|r| Sniffer {
                ...
                parse: |s| {
                    ...
                    ms_str.parse::<i64>().ok().and_then(|ms| {
                        DateTime::from_timestamp(ms / 1_000, ((ms % 1_000) as u32) * 1_000_000)
                    })
                },
            }),
            // ------------------------------------------------------------------
            // Tier 14 -- Dot-separated date-only: DD.MM.YYYY (no time component).
            // Example: 15.02.2026 some event
            // Treated as midnight UTC.  Day/month may be 1 or 2 digits.
            // Placed after all date+time tiers so a line with both date and time
            // is handled by the appropriate earlier tier (e.g. Tier 5).
            // ------------------------------------------------------------------
            try_re(r"\b\d{1,2}\.\d{1,2}\.\d{4}\b").map(|r| Sniffer {
                re: r,
                parse: |s| {
                    // Try DD.MM.YYYY then MM.DD.YYYY as fallback.
            ...
            // ------------------------------------------------------------------
            // Tier 15 -- Time-only: HH:MM:SS[.mmm] with no date.
            // Example: 14:30:22.123 [thread-1] request accepted
            // Date is injected from today's UTC date (best-effort; low
            // confidence).  Placed last so any line that also contains a date
            // has already been matched by an earlier tier.
            // ------------------------------------------------------------------
            try_re(r"\b\d{2}:\d{2}:\d{2}(?:[.,]\d+)?\b").map(|r| Sniffer {
                re: r,
                parse: |s| {
...
"#;
    ...
    /// Tier 13: Unix epoch milliseconds (13 digits) at line start.
    #[test]
    fn test_sniff_unix_epoch_millis() {
        // 1705329022123 ms = 2024-01-15 14:30:22 UTC
        let result = sniff_timestamp("1705329022123 server started");
        assert!(result.is_some(), "13-digit epoch should be parsed");
    ...
    /// Tier 13: 10-digit epoch must NOT be mis-parsed as 13-digit.
    #[test]
    fn test_sniff_epoch_seconds_not_confused_with_millis() {
        // 1705329022 is 10 digits -- should parse as seconds, not millis.
        let result = sniff_timestamp("1705329022 event");
        assert!(result.is_some());
        let dt = result.unwrap();
        // As seconds: 2024-01-15 14:30:22.  As millis it would be 1970-01-20.
        assert_eq!(
            dt.format("%Y").to_string(),
            "2024",
            "10-digit epoch must be treated as seconds, not milliseconds"
        );
    }

    /// Tier 14: dot-separated date-only (DD.MM.YYYY).
    #[test]
    fn test_sniff_dot_date_only() {
        let result = sniff_timestamp("15.02.2026 system rebooted");
        assert!(result.is_some(), "dot date-only should be parsed");
        let dt = result.unwrap();
        assert_eq!(dt.format("%Y-%m-%d").to_string(), "2026-02-15");
        // Treated as midnight.
        assert_eq!(dt.format("%H:%M:%S").to_string(), "00:00:00");
    }

    /// Tier 14: single-digit day/month (D.M.YYYY).
    #[test]
    fn test_sniff_dot_date_only_single_digit() {
        let result = sniff_timestamp("5.3.2026 short date");
        assert!(
            result.is_some(),
            "single-digit dot date-only should be parsed"
        );
        let dt = result.unwrap();
        assert_eq!(dt.format("%Y-%m-%d").to_string(), "2026-03-05");
    }

    /// Tier 15: time-only HH:MM:SS at anywhere in the line.
    #[test]
    fn test_sniff_time_only() {
        // A line with a time but no date (e.g. some structured log formats).
        let result = sniff_timestamp("14:30:22 [thread-1] request accepted");
        assert!(
            result.is_some(),
            "time-only HH:MM:SS should be parsed and given today's date"
        );
        let dt = result.unwrap();
        assert_eq!(
    ...
    /// Tier 15: time-only with comma-milliseconds.
    #[test]
    fn test_sniff_time_only_with_millis() {
        let result = sniff_timestamp("INFO 08:15:01.456 startup complete");
        assert!(
            result.is_some(),
            "time-only with dot-millis should be parsed"
        );
        let dt = result.unwrap();
        assert_eq!(dt.format("%H:%M:%S").to_string(), "08:15:01");
    }

    /// Tier 15 must NOT fire when a full date+time is present (Tier 5 wins).
    #[test]
    fn test_sniff_full_datetime_preempts_time_only() {
        // 26.02.2026 22:07:56 has both date and time; Tier 5 handles it.
        // The returned year must be 2026, not today's year (which Tier 15 would inject).
        let result = sniff_timestamp("26.02.2026 22:07:56 some event");
        assert!(result.is_some());
        let dt = result.unwrap();
        assert_eq!(
            dt.format("%Y-%m-%d").to_string(),
            "2026-02-26",
            "full datetime should be handled by Tier 5, not the time-only fallback"
        );
    }
