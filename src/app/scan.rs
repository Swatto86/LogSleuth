...
///
/// Protects the scan against I/O stalls (slow drives, antivirus, etc.) so
/// Cancel takes effect promptly.  The spawned I/O thread is not forcibly
/// killed but exits on its own once the OS I/O timeout fires.
...
impl ScanManager {
    ...
    ///
    /// `parse_path_filter` — when `Some`, only files whose paths appear in this
    /// set will be fully read and parsed.  All other discovered files are still
    /// included in the file list (with profile assigned by filename only) but
    /// their `parsing_skipped` flag is set so the UI can offer a follow-up
    /// "Parse skipped files" action.  Pass `None` to parse everything (the
    /// default behaviour for a fresh scan with no active file filter).
    pub fn start_scan(
        &mut self,
        root: PathBuf,
    ...
    ///
    /// `parse_path_filter` — when `Some`, only files whose paths appear in this
    /// set are fully read and parsed; all other files receive a filename-only
    /// profile assignment and have `parsing_skipped = true`.  Pass `None` to
    /// parse every file in the list (the default for user-initiated opens).
    /// Pass `Some(HashSet::new())` for directory-watcher discovers where the
    /// user has not explicitly opted in to loading the files (opt-in model).
...
// =============================================================================
// Process-scoped rayon thread pool
// =============================================================================

/// Return the process-scoped rayon thread pool, building it on first call.
///
/// A single long-lived pool is reused for all scan invocations rather than
/// constructing a new one on each scan button click.  Creating a ThreadPool
/// spawns OS threads and tears them down again at drop time, adding 50-200 ms
/// of overhead per scan and temporarily spiking RSS.  Reusing the pool
/// eliminates that cost entirely.
///
/// Pool size is capped at `MAX_SCAN_THREADS` to prevent excessive context
/// switching on high-core-count workstations.
fn scan_thread_pool() -> &'static rayon::ThreadPool {
    ...
    POOL.get_or_init(|| {
        let num_cpus = std::thread::available_parallelism()
            .map(|n| n.get())
            .unwrap_or(4);
        let parallelism = num_cpus.min(crate::util::constants::MAX_SCAN_THREADS);
        tracing::debug!(num_cpus, parallelism, "Building static scan thread pool");
        rayon::ThreadPoolBuilder::new()
            ...
            .unwrap_or_else(|e| {
                tracing::warn!(
                    error = %e,
                    "Failed to build custom rayon pool; falling back to default"
                );
                rayon::ThreadPoolBuilder::new()
                    .build()
                    .expect("rayon default pool should always build")
            })
    })
}
...
///
/// `parse_path_filter`: when `Some`, only files whose paths are in the set are
/// fully read and parsed.  All other files receive a filename-only profile
/// assignment and are marked `parsing_skipped = true`.  Pass `None` to parse
/// all discovered files (default).
...
fn run_parse_pipeline(
    ...
    entry_id_start: u64,
    // When Some, only files in this set are parsed; others are skipped.
    parse_path_filter: Option<&std::collections::HashSet<PathBuf>>,
) {
    macro_rules! send {
        ($msg:expr) => {
    ...
    // overlapping parsing CPU work with disk I/O.
    //
    // Key gains:
    //   - Overlaps I/O and parse CPU across files via rayon parallelism
    //   - Timeout-guarded reads protect rayon workers from I/O stalls
    //   - Entry IDs are assigned sequentially after collection to maintain
    //     global uniqueness without cross-thread coordination
    //   - Files are sorted by parent directory to improve OS metadata prefetch

    let total_files = discovered_files.len();
    let scan_start = Instant::now();

    // Pre-sort files by parent directory so files in the same folder are
    // processed on nearby rayon iterations, improving OS metadata prefetch.
    // On most filesystems the improvement is modest but harmless.
    ...
    struct FileResult {
        ...
        error_count: usize,
        /// Whether this file was skipped due to an active `parse_path_filter`.
        /// Propagated to `DiscoveredFile::parsing_skipped` so the UI can show
        /// a "(not parsed)" indicator and offer a follow-up parse action.
    ...
    // Obtain the process-scoped rayon thread pool (built once, reused for all
    // subsequent scans).  Building a new ThreadPool on each scan call spawns
    // and tears down OS threads on every button click, adding ~50-200 ms of
    // thread management overhead and increasing peak RSS while the old pool's
    // threads drain.  A static pool avoids all of that.
            ...
            if let Some(filter) = parse_path_filter {
                if !filter.contains(&file.path) {
                    ...
                    let (profile_id, detection_confidence) = if let Some(detection) =
                        profile::auto_detect(file_name, &[], &profiles)
                    {
    ...
    // Also propagate the parsing_skipped flag so the UI can show a "(not
    // parsed)" indicator for files excluded by the parse-path filter.
    for result in &file_results {
        ...
        if result.parsing_skipped {
            discovered_files[result.idx].parsing_skipped = true;
        }
    }
...
fn run_files_scan(
    ...
    // Use a per-file timeout-guarded stat so that files
    // on a slow or temporarily unavailable path never hang the scan thread
    // indefinitely (Rule 11).
    ...
    match rx.recv_timeout(Duration::from_secs(FILE_READ_TIMEOUT_SECS)) {
        ...
        Err(_) => {
            tracing::warn!(
                ...
                "File read timed out; skipping file"
            );
            Err(io::Error::new(
                io::ErrorKind::TimedOut,
                format!("File read timed out after {FILE_READ_TIMEOUT_SECS}s"),
            ))
        }
    }
}
