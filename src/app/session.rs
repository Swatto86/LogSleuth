...
pub struct SessionData {
    ...
    /// Maximum number of live-tail entries held in the ring-buffer before the
    /// oldest tail entries are evicted.  Older sessions that pre-date this
...
fn default_max_tail_buffer_entries() -> usize {
    crate::util::constants::DEFAULT_MAX_TAIL_BUFFER_ENTRIES
}
...
pub struct PersistedFilter {
    ...
    /// Exclusion text filter: entries containing this substring are hidden.
    /// Empty = no exclusion.
    #[serde(default)]
    ...
    /// Deduplication mode (Off / Exact / Normalized).
...
mod tests {
    ...
    fn test_session_save_load_round_trip() {
        ...
        // Dedup mode must round-trip.
        assert_eq!(
            loaded.filter.dedup_mode,
            DedupMode::Normalized,
            "dedup_mode must survive round-trip"
        );
    }
