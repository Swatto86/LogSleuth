...
fn main() {
    ...
    if let Some(session) = app::session::load(&session_file) {
        ...
        if has_scan {
            // Do NOT trigger a scan automatically.  Session metadata
            // (bookmarks, filters, colours, last scan path) is restored so
            // everything is ready, but the user must explicitly click
            // "Open Directory..." or "Open Log(s)..." to load entries.
            let dir = state
                .scan_path
                .as_ref()
