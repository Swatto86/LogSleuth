...
> **Status**: Increment 33 complete — timeline sort order toggle added: ascending (oldest first) / descending (newest first) button above the scroll area; `sort_descending: bool` user preference (not cleared on `clear()`); `toggle_sort_direction()` method; display-layer reversal via `actual_idx` in `show_rows`; `stick_to_bottom` disabled in descending mode. 100 tests passing (71 unit + 29 E2E), zero clippy warnings, clean fmt.
