...
> **Status**: Increment 28 complete — (1) DirWatcher `modified_since` filter: real-time directory watcher now honours the discovery date filter so only files with an OS mtime ≥ the filter date are added to the session; (2) severity row highlight replaced with a 2 px underline in the severity colour (removed full-row background tint and `severity_bg_colour` from theme); (3) source-file checklist virtualised with `show_rows` for O(visible_rows ≈ 9) per-frame cost at any file count; (4) "Select All" button scoped correctly to the visible (search-filtered) subset. 79 tests passing, zero clippy warnings.
