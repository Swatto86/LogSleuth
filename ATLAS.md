...
> **Status**: Increment 31 complete — (29) sidebar restructured: tab-based (Files | Filters), resizable 300–800 px, unified file list with inline checkboxes/solo/profile, collapsible scan-controls header; (30) five bug fixes with regression tests: Open-Directory date-filter ignored, append-scan duplicate IDs, ghost correlation tints, `discovery_date_input` not persisted, menu items enabled during scan; (31) font I/O moved before `eframe::run_native` (DevWorkflow Rule 16 compliance, eliminates white-flash startup). 98 tests passing (69 unit + 29 E2E), zero clippy warnings, clean fmt.
...
|   |   |   +-- discovery.rs     # Files tab renderer: (1) collapsible scan-controls header (CollapsingHeader, default_open=true) containing path label, date filter (YYYY-MM-DD HH:MM:SS + quick-fill buttons), Open Directory / Open Log(s) / Clear Session buttons; (2) unified file list with count badge, All/Live-Tail/search-box/Select-All-None controls, virtual-scroll via show_rows at ROW_HEIGHT — each row: dot + checkbox + filename + solo button + right-aligned profile label; hover shows full path + size + profile; source-file filter state driven directly from the file list (replaces separate duplicate list that was in filters.rs)
