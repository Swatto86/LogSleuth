...
## Increment 42 -- AI Operating Contract full compliance audit (Part 1)

**Scope**: Complete audit of the codebase against the DevWorkflow AI Operating Contract (Parts A-E). Three concrete defects identified and fixed.

### (a) Rule 18 -- Fix `Invoke-Git` in `update-application.ps1`

**Problem**: `Invoke-Git` was declared as an advanced function with a named `[Parameter(ValueFromRemainingArguments)]` `[string[]]$Arguments` parameter. PowerShell advanced functions inject common parameters (-Debug, -Verbose, etc.) and allow prefix abbreviation; short git flags like `-d` were silently swallowed by PowerShell and never forwarded to git (e.g. `Invoke-Git tag -d v1.0.0` ran `git tag v1.0.0` -- creating instead of deleting the tag). The function also did not capture stderr from git, and its throw message contained only the exit code with no git output.

**Fix**: Rewrote `Invoke-Git` as a simple (non-advanced) function using the automatic `$args` variable. `$args` performs no named-parameter binding so every positional argument is forwarded verbatim. Added `2>&1` to capture stderr; the throw message now includes both exit code and the full git output. Updated all call sites to pass individual string arguments rather than `@(array)` literals. Now compliant with Rule 18 Invoke-Git CRITICAL requirement.

### (b) Part E -- Add `workflow_dispatch` trigger to CI/release workflows

**Problem**: Neither `ci.yml` nor `release.yml` had a `workflow_dispatch:` trigger, preventing manual triggering for verification as required by Part E.

**Fix**: Added `workflow_dispatch:` to the `on:` block of both workflows.

### (c) Rule 5 / INV-10 -- Implement log-file support in `util::logging::init()`

**Problem**: `ATLAS.md` documented that `util::logging` would write structured logs to a file when `[logging] file` was set in `config.toml`, but the code silently ignored the `log_file` parameter with a TODO comment. Atlas/code contradiction = INV-10 defect.

**Fix**: Implemented the file layer in `util::logging::init()` using a two-layer `tracing-subscriber` registry. The stderr layer is always active. When `log_file` is `Some`, a second layer backed by an appending `Mutex<File>` is added. If the file cannot be opened, the function prints a warning to stderr and continues without the file layer (fail-open per Rule 11).

### Changes

- [x] `update-application.ps1` -- `Invoke-Git` rewritten as simple `$args`-based function; all 10 call sites updated; throw message includes exit code + git output.
- [x] `.github/workflows/ci.yml` -- Added `workflow_dispatch:` to `on:` block.
- [x] `.github/workflows/release.yml` -- Added `workflow_dispatch:` to `on:` block.
- [x] `src/util/logging.rs` -- `init()` now builds a two-layer registry; `file_writer: Option<Mutex<File>>` layer added when `log_file` is `Some` (fail-open on open error).
- [x] `ATLAS.md` -- Status block updated to Increment 42; Section 10 (Debug Mode) updated to describe the dual-layer file logging; util/logging.rs description updated.

**Test results: 109 unit + 30 E2E = 139 total, all passing. Zero clippy warnings, clean fmt.**

---

## Increment 43 -- AI Operating Contract full compliance audit (Part 2)

**Scope**: Full compliance re-audit after Increment 42. One remaining Rule 2 violation found and fixed; one documentation inconsistency (INV-10) resolved.

### (a) Rule 2 -- Fix non-ASCII bytes in profile TOML files

**Problem**: 10 of the 22 built-in format profile `.toml` files contained non-ASCII bytes. The characters were cosmetic Unicode box-drawing glyphs (U+2500 HORIZONTAL, U+2502 VERTICAL) and typographic dashes (U+2014 EM DASH, U+2013 EN DASH) and arrows (U+2192 RIGHT ARROW) used inside TOML `#` comment lines as visual section separators. Rule 2 (Config/CI file encoding) requires "any YAML/TOML/JSON/INI config" to be "pure ASCII (bytes 0x00-0x7F only)."

**Fix**: Replaced each non-ASCII character with its ASCII equivalent in all 10 affected files:
- U+2500 `--` (box-drawing horizontal -> hyphen-hyphen)
- U+2502 `|` (box-drawing vertical -> pipe)
- U+2014 `--` (em dash -> double hyphen)
- U+2013 `-` (en dash -> single hyphen)
- U+2192 `->` (right arrow -> ASCII arrow)

Affected files: `apache_combined.toml`, `exchange_tracking.toml`, `intune_ime.toml`, `kubernetes_klog.toml`, `postgresql_log.toml`, `sccm_cmtrace.toml`, `tomcat_catalina.toml`, `veeam_vbr.toml`, `windows_cluster.toml`, `windows_firewall.toml`.

### (b) INV-10 -- Resolve PROGRESS.md / ATLAS.md documentation gap

**Problem**: ATLAS.md status block documented Increment 42 as complete, but PROGRESS.md had no Increment 42 entry. Atlas and progress tracker contradicted each other (INV-10 violation).

**Fix**: Added the missing Increment 42 entry to PROGRESS.md (above), restoring consistency between ATLAS.md and PROGRESS.md.

### Changes

- [x] `profiles/apache_combined.toml` -- Non-ASCII comment chars replaced with ASCII.
- [x] `profiles/exchange_tracking.toml` -- Non-ASCII comment chars replaced with ASCII.
- [x] `profiles/intune_ime.toml` -- Non-ASCII comment chars replaced with ASCII.
- [x] `profiles/kubernetes_klog.toml` -- Non-ASCII comment chars replaced with ASCII.
- [x] `profiles/postgresql_log.toml` -- Non-ASCII comment chars replaced with ASCII.
- [x] `profiles/sccm_cmtrace.toml` -- Non-ASCII comment chars replaced with ASCII.
- [x] `profiles/tomcat_catalina.toml` -- Non-ASCII comment chars replaced with ASCII.
- [x] `profiles/veeam_vbr.toml` -- Non-ASCII comment chars replaced with ASCII.
- [x] `profiles/windows_cluster.toml` -- Non-ASCII comment chars replaced with ASCII.
- [x] `profiles/windows_firewall.toml` -- Non-ASCII comment chars replaced with ASCII.
- [x] `PROGRESS.md` -- Added Increment 42 entry; added this Increment 43 entry.
- [x] `ATLAS.md` -- Status block updated to Increment 43.

**Test results: 109 unit + 30 E2E = 139 total, all passing. Zero clippy warnings, clean fmt.**

---

## Increment 49 -- Deduplication filter
**Status: COMPLETE**

**Scope**: Post-filter deduplication step that collapses repeated log messages into a single representative entry. Three modes: Off (default), Exact match, Normalized. Normalized mode replaces variable data (GUIDs, IPv4/IPv6 addresses, 0x-prefixed hex strings, bare numbers) with placeholder tokens before comparison. Scoped per source file; latest-timestamp entry survives per group. Purple count badge in timeline, collapsible occurrences section in detail panel, session persistence.

### Changes

- [x] `src/core/filter.rs` -- Added `DedupMode` enum (Off/Exact/Normalized) with serde, `DedupInfo` struct, `NormRegexes` with `OnceLock`-cached compiled regexes, `normalize_message()` function, `apply_dedup()` function, `dedup_mode: DedupMode` field on `FilterState`, updated `is_empty()`. 17 new unit tests.
- [x] `src/app/state.rs` -- Added `dedup_info: HashMap<usize, DedupInfo>` field; integrated dedup call in `apply_filters()` after activity-window post-filter; live-tail `extend_filtered_for_range()` falls back to full rebuild when dedup active; cleared in `clear()`; wired to `save_session()` and `restore_from_session()`.
- [x] `src/ui/panels/filters.rs` -- Added dedup dropdown ComboBox (Off/Exact match/Normalized) between exclusion filter and time range; purple stats line when active showing unique count and hidden duplicates.
- [x] `src/ui/panels/timeline.rs` -- Purple `(xN)` count badge appended to row text via LayoutJob when dedup_info is present.
- [x] `src/ui/panels/detail.rs` -- Collapsible "Occurrences (N)" section with purple header; striped grid showing Timestamp/File/Line for all collapsed entries; first-seen timestamp shown above grid.
- [x] `src/app/session.rs` -- Added `dedup_mode: DedupMode` to `PersistedFilter` with `#[serde(default)]` for backward compatibility; round-trip test updated.
- [x] `LogSleuth-Specification.md` -- Added FILT-11 (deduplication filter requirement); updated SP-02 to include dedup mode.
- [x] `ATLAS.md` -- Status block updated to Increment 49; filter.rs, state.rs, session.rs, filters.rs, timeline.rs, detail.rs descriptions updated; `apply_dedup` added to Internal APIs table.
- [x] `PROGRESS.md` -- Added this Increment 49 entry.

**Test results: 133 unit + 30 E2E = 163 total, all passing. Zero clippy warnings, clean fmt.**
...
- [x] **Complete performance audit and improvements** (2026-03-02):
  - `parser.rs`: Hoisted `PathBuf`/`profile_id` clones out of the per-line loop (eliminates O(n) redundant allocations on every parse). Merged post-loop truncation + timestamp-sniff passes into a single O(n) traversal. Raw-mode entries reuse the `message` String as `raw_text` (one allocation instead of two).
  - `filter.rs`: Eliminated `to_lowercase()` String allocations for `message`/`thread`/`component` on every entry in every filter pass. Added `contains_ci` fast-path for ASCII-only log lines using `eq_ignore_ascii_case`. Updated `fuzzy_match` to accept pre-lowercased query and mixed-case text (no per-character lowercasing needed). Pre-sized `filtered_indices` Vec with a heuristic capacity to avoid multiple reallocations.
  - `profile.rs` / `model.rs`: Pre-compiled glob patterns in `FormatProfile::compiled_file_patterns` at profile-load time. `auto_detect` now uses pre-compiled `glob::Pattern` values instead of parsing and compiling glob strings on every file evaluation (eliminates O(profiles x files) compilations per scan).
  - `state.rs`: Guard `active_files` HashSet construction with an `is_empty()` short-circuit to avoid allocating on every `apply_filters` call when no files are loaded.
  - `scan.rs`: Replaced per-scan `rayon::ThreadPoolBuilder::build()` with a process-scoped static pool via `OnceLock`. Eliminates OS thread spawn/teardown overhead (~50-200ms per scan) and RSS spike from old pool drain.
  - `Cargo.toml`: Added `codegen-units = 1` and `panic = "abort"` to `[profile.release]` for better inlining and smaller release binary.
